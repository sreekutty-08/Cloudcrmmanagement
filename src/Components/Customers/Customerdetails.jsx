...
function CustomerDetails() {
  ...
  const fetchCustomerDetails = useCallback(async () => {
    ...
    try {
      ...
      setCustomer(combinedCustomer);

      setResources(resourceData || []);

      setFormData({
        customerId: combinedCustomer.customer_id || "",

        companyName: combinedCustomer.company_name || "",

        accountManager:
          combinedCustomer.account_manager || "",

        status: combinedCustomer.status || "ACTIVE",

        country: combinedCustomer.country || "",

        website: combinedCustomer.website || "",

        email: combinedCustomer.email || "",
      ...
      setIpAddresses(
        Array.isArray(customerData.ip_addresses) &&
          customerData.ip_addresses.length > 0
  ...
  const openEditModal = () => {
    ...
    setIpAddresses(
      Array.isArray(customer.ip_addresses) &&
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const cleanIpAddresses = ipAddresses
        .map((ip) => ip.trim())
        .filter(Boolean);
